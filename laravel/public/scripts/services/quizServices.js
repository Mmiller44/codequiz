...
// Returns object to controller. If object is empty, controller handles the next API call.
	...
	$rootScope.$on('storeAnswerEvent', function (event, data) {
		...
		var addingQuestion = answerAPI.get({userID: data.userID, userQuizID: data.userQuizID, questionID: data.questionID, userAnswer: data.userAnswer, correct: data.right}, function() {
